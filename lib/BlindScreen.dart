...
class _BlindScreenState extends State<BlindScreen> with WidgetsBindingObserver {
  final FlutterTts flutterTts = FlutterTts();
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      // Resume TTS/STT when coming back from Registration/Login
      Future.delayed(const Duration(milliseconds: 300), () {
        _startVoiceAssistant();
      });
    }
    ...
    SpeechService().init((words) async {
      ...
      if (commandText.contains("login")) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => LoginScreen()),
        ).then((_) => _startVoiceAssistant());
      } else if (commandText.contains("registration") || commandText.contains("register")) {
        SpeechService().stopListening();
      ...
      } else if (commandText.contains("back") || commandText.contains("home")) {
        _goBack();
      } else if (commandText.isNotEmpty) {
        // Not recognized
        await _speak(
          "Sorry, I did not recognize that. Please say again.",
          onComplete: () => _listenToCommand(),
        );
      }
    });
